...
import {
  ...
  Sparkles, Send, ExternalLink, Maximize, Minimize, VolumeX, Volume2,
  Subtitles, Sidebar, PictureInPicture, RefreshCw, Square, LogOut
} from 'lucide-react';
...
const DEFAULT_CC = { size:'1.125rem', bg:'rgba(0,0,0,0.75)', color:'#ffffff', font:'ui-sans-serif,system-ui,-apple-system,sans-serif', edgeStyle:'dropshadow', animation:'fade', offset:-5.0 };
const DEFAULT_SETTINGS = { apiKey:DEFAULT_TMDB_API_KEY, omdbApiKey:'93a6d7d6', sourceKey:'vidking', requireProfile:false, algoPrefs:{excluded:[],boosted:[]}, ccSettings:DEFAULT_CC };

const getTrailerUrl = (id) => `https://tube.rvere.com/embed?v=${id}&autoplay=1&fs=1&modestbranding=1&color=white&rel=0&iv_load_policy=3`;
const getTextShadow = (s) => ({ dropshadow:'0px 2px 6px rgba(0,0,0,0.9),0px 4px 12px rgba(0,0,0,0.6)', raised:'-1px -1px 0 rgba(255,255,255,0.2),1px 1px 0 rgba(0,0,0,0.8)', depressed:'1px 1px 0 rgba(255,255,255,0.3),-1px -1px 0 rgba(0,0,0,0.8)', outline:'-1.5px -1.5px 0 #000,1.5px -1.5px 0 #000,-1.5px 1.5px 0 #000,1.5px 1.5px 0 #000' }[s] || 'none');

const SOURCES = {
  vidlink:    { name:"VidLink",    url:"https://vidlink.pro",           type:"Primary"  },
  vidking:    { name:"VidKing",    url:"https://www.vidking.net/embed", type:"Fast"     },
  vidsrc:     { name:"VidSrc",     url:"https://vidsrc.net/embed",      type:"Backup 1" },
  embedsu:    { name:"Embed.su",   url:"https://embed.su/embed",        type:"Backup 2" },
  vidsrcpro:  { name:"VidSrc Pro", url:"https://vidsrc.pro/embed",      type:"Backup 3" },
  superembed: { name:"SuperEmbed", url:"https://www.2embed.cc/embed",   type:"Backup 4" },
};

const GENRES = {
  movie:[{id:28,name:"Action"},{id:12,name:"Adventure"},{id:16,name:"Animation"},{id:35,name:"Comedy",exclude:"16"},{id:80,name:"Crime"},{id:99,name:"Documentary"},{id:18,name:"Drama"},{id:10751,name:"Family"},{id:14,name:"Fantasy"},{id:27,name:"Horror"},{id:878,name:"Sci-Fi"},{id:53,name:"Thriller"}],
  tv:[{id:"10759_act",realId:10759,name:"Action",keywords:"9715|83|207317"},{id:"10759_adv",realId:10759,name:"Adventure",keywords:"9717|2095"},{id:16,name:"Animation"},{id:35,name:"Comedy",exclude:"16"},{id:80,name:"Crime"},{id:99,name:"Documentary"},{id:18,name:"Drama"},{id:"10765_sci",realId:10765,name:"Sci-Fi",keywords:"9882|33643|4344"},{id:"10765_fan",realId:10765,name:"Fantasy",keywords:"1422|2095|13084"},{id:10768,name:"War & Politics"}],
};
const ALL_UNIQUE_GENRES = [];
[...GENRES.movie,...GENRES.tv].forEach(g=>{ if(!ALL_UNIQUE_GENRES.some(u=>u.name===g.name)) ALL_UNIQUE_GENRES.push({id:g.realId||g.id,name:g.name}); });
...
const initHls = (video,src,isMain,cb) => {
  ...
  if (window.Hls.isSupported()) {
    const hls = new window.Hls({ maxMaxBufferLength:isMain?30:10, maxBufferSize:isMain?60e6:15e6, liveSyncDurationCount:3, capLevelToPlayerSize:true, autoLevelCapping:isMain?-1:0, enableWorker:true, renderTextTracksNatively:true, enableCEA708Captions:true, enableWebVTT:true });
...
const lockScroll   = () => { if (typeof window!=='undefined') { window.scrollLocks=(window.scrollLocks||0)+1; document.body.style.overflow='hidden'; } };
const unlockScroll = () => { if (typeof window!=='undefined') { window.scrollLocks=Math.max((window.scrollLocks||1)-1,0); if(!window.scrollLocks) document.body.style.overflow=''; } };
...
// ─── LOCAL STORAGE HOOK (UI-only prefs) ───────────────────────────────────
...
const useDebounce = (val,delay) => { const [d,setD]=useState(val); useEffect(()=>{const h=setTimeout(()=>setD(val),delay);return()=>clearTimeout(h);},[val,delay]); return d; };

// ─── FIRESTORE SYNC HOOK ───────────────────────────────────────────────────
const safeLS = { get:(k,fb)=>{try{const v=localStorage.getItem(k);return v?JSON.parse(v):fb;}catch{return fb;}}, set:(k,v)=>{try{localStorage.setItem(k,JSON.stringify(v));}catch{}} };
const LEGACY_MAP = { apiKey:'onyx_tmdb_key_v3', omdbApiKey:'onyx_omdb_key_v1', sourceKey:'onyx_source_v7', requireProfile:'onyx_require_profile', algoPrefs:'onyx_algo_v1', ccSettings:'onyx_cc_settings' };
...
const useFirestoreData = (uid) => {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try {
        const [ss,hs,ws]=await Promise.all([getDoc(doc(db,'users',uid,'data','settings')),getDoc(doc(db,'users',uid,'data','history')),getDoc(doc(db,'users',uid,'data','watchlist'))]);
        if (!ok) return;
  ...
  const save = useCallback((docId,getVal)=>{
    ...
    timers.current[docId]=setTimeout(async()=>{
      try{ const v=getVal(); await setDoc(doc(db,'users',uid,'data',docId),docId==='settings'?v:{items:v}); }catch(e){console.error('Firestore save:',e);}
    },1500);
  },[uid]);
...
const LoginScreen = () => {
  ...
  const handle = async(e)=>{ e.preventDefault(); setLoading(true); setError('');
    try{await signInWithEmailAndPassword(auth,email,password);}
    catch{setError('Invalid email or password.');}
    setLoading(false);
  };
  return (
    <div className="min-h-screen bg-black flex items-center justify-center font-sans px-6">
      <div className="w-full max-w-md">
        <div className="flex justify-center mb-10"><div className="w-16 h-16 bg-white rounded-3xl flex items-center justify-center shadow-2xl"><Play className="text-black w-8 h-8 fill-black ml-1"/></div></div>
        <h1 className="text-4xl font-bold text-white text-center mb-2 tracking-tight">Welcome back</h1>
        <p className="text-white/50 text-center mb-10 text-base">Sign in to access your stream</p>
        <form onSubmit={handle} className="space-y-4">
          <input type="email" placeholder="Email address" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-white/10 border border-white/10 text-white px-6 py-4 rounded-2xl outline-none focus:border-[#19446C] focus:ring-2 focus:ring-[#19446C]/50 placeholder:text-white/40 text-lg transition-all" required autoComplete="email"/>
          <input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-white/10 border border-white/10 text-white px-6 py-4 rounded-2xl outline-none focus:border-[#19446C] focus:ring-2 focus:ring-[#19446C]/50 placeholder:text-white/40 text-lg transition-all" required autoComplete="current-password"/>
          {error && <p className="text-red-400 text-sm text-center bg-red-500/10 rounded-xl p-3 border border-red-500/20">{error}</p>}
          <button type="submit" disabled={loading} className="w-full bg-white text-black font-bold text-lg py-4 rounded-2xl hover:bg-white/90 transition-all disabled:opacity-50 flex items-center justify-center gap-3 shadow-lg">
            {loading?<div className="w-6 h-6 border-4 border-black/20 border-t-black rounded-full animate-spin"/>:'Sign In'}
          </button>
        </form>
        <p className="text-white/30 text-xs text-center mt-8">Access is restricted to invited users only.</p>
      </div>
    </div>
  );
};
...
// ─── TOP NAV ──────────────────────────────────────────────────────────────
const TopNavigation = React.memo(({activeTab,setActiveTab,selectedGenre,setSelectedGenre,focusMode,onSignOut})=>{
  ...
  return (
    <header className={cn("fixed top-0 left-0 w-full z-[70] transition-all duration-500 pointer-events-none flex flex-col items-center hardware-accel",focusMode?"-translate-y-full opacity-0":"translate-y-0 opacity-100",scrolled&&!focusMode?"bg-black/80 backdrop-blur-3xl border-b border-white/5":"bg-gradient-to-b from-black/80 via-black/30 to-transparent")}>
      <nav className="w-full flex items-center justify-between px-4 md:px-8 lg:px-12 xl:px-16 h-20 pointer-events-auto max-w-[1800px]">
        <div className="flex items-center gap-6 md:gap-10">
          <div onClick={()=>setActiveTab('home')} className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center cursor-pointer shrink-0 hover:scale-105 transition-transform shadow-lg"><Play className="text-black w-5 h-5 fill-black ml-0.5"/></div>
          <div className="hidden md:flex items-center gap-8">
            {mainTabs.map(t=><button key={t.id} onClick={()=>setActiveTab(t.id)} className={cn("text-[15px] transition-all tracking-wide outline-none rounded-md px-2 py-1 relative",activeTab===t.id?"text-white font-bold drop-shadow-lg":"text-white/50 font-semibold hover:text-white/80")}>{t.label}{activeTab===t.id&&<div className="absolute -bottom-1 left-0 w-full h-[2.5px] bg-white rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)]"/>}</button>)}
          </div>
        </div>
        <div className="flex items-center gap-1.5 md:gap-4 shrink-0 overflow-hidden">
          <div className="md:hidden flex items-center overflow-x-auto gap-5 mr-4 max-w-[50vw] fade-edge-right">
            {mainTabs.map(t=><button key={t.id} onClick={()=>setActiveTab(t.id)} className={cn("text-[13px] transition-all whitespace-nowrap px-2 py-2 outline-none rounded-md relative",activeTab===t.id?"text-white font-bold":"text-white/50 font-semibold hover:text-white/80")}>{t.label}{activeTab===t.id&&<div className="absolute bottom-0 left-0 w-full h-[2.5px] bg-white rounded-full"/>}</button>)}
          </div>
          <div className="flex gap-1 md:gap-2">
            {utilTabs.map(t=><button key={t.id} onClick={()=>setActiveTab(t.id)} className={cn("p-2.5 rounded-full flex items-center justify-center transition-all shrink-0 outline-none",activeTab===t.id?"bg-[#19446C] text-white shadow-lg":"text-white/70 hover:text-white hover:bg-white/10")} title={t.label}><t.icon className="w-4 h-4 md:w-5 md:h-5"/></button>)}
            <button onClick={onSignOut} className="p-2.5 rounded-full flex items-center justify-center transition-all shrink-0 outline-none text-white/70 hover:text-white hover:bg-white/10" title="Sign Out"><LogOut className="w-4 h-4 md:w-5 md:h-5"/></button>
          </div>
        </div>
      </nav>
      <div className={cn("w-full overflow-hidden transition-all duration-300 pointer-events-auto flex justify-center max-w-[1800px]",showSub&&!focusMode?"max-h-16 opacity-100 pb-4":"max-h-0 opacity-0 pb-0")}>
        <div className="flex items-center overflow-x-auto gap-6 md:gap-8 px-4 md:px-8 lg:px-12 xl:px-16 w-full fade-edge-right">
          <button onClick={()=>setSelectedGenre('All')} className={cn("py-1.5 px-2 text-[13px] font-semibold transition-all whitespace-nowrap outline-none rounded-md relative",selectedGenre==='All'?"text-white":"text-white/50 hover:text-white/80")}>All{selectedGenre==='All'&&<div className="absolute -bottom-0.5 left-0 w-full h-[2px] bg-white rounded-full"/>}</button>
          {genres.map(g=><button key={g.id} onClick={()=>setSelectedGenre(g.id)} className={cn("py-1.5 px-2 text-[13px] font-semibold transition-all whitespace-nowrap outline-none rounded-md relative",selectedGenre===g.id?"text-white":"text-white/50 hover:text-white/80")}>{g.name}{selectedGenre===g.id&&<div className="absolute -bottom-0.5 left-0 w-full h-[2px] bg-white rounded-full"/>}</button>)}
        </div>
      </div>
    </header>
  );
...
const MediaCard = React.memo(({media,onClick,size="normal",showRank=null})=>{
  const img=media.poster_path?`${IMAGE_BASE_URL}${POSTER_SIZE}${media.poster_path}`:'https://via.placeholder.com/500x750?text=No+Image';
  const w=size==="large"?"w-40 md:w-60":size==="grid"?"w-full":"w-32 md:w-44";
  return (
    <div onClick={()=>onClick(media)} className={cn("relative group cursor-pointer flex-shrink-0 transition-all duration-300 z-10 hover:z-50 outline-none focus:z-50 rounded-2xl md:rounded-[1.25rem] select-none snap-start scroll-ml-4 md:scroll-ml-8 lg:scroll-ml-12 xl:scroll-ml-16",w)} tabIndex={0} onKeyDown={e=>{if(e.key==='Enter')onClick(media);}}>
      ...
      <div className="aspect-[2/3] rounded-2xl md:rounded-[1.25rem] overflow-hidden bg-white/5 transition-all duration-300 ease-out group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)] border border-transparent group-hover:border-white/30 group-focus:border-white/60 relative z-20 hardware-accel">
        <img src={img} alt={media.title||media.name} className="w-full h-full object-cover transition-transform duration-700 pointer-events-none" loading="lazy" decoding="async" draggable="false" onError={e=>{e.target.onerror=null;e.target.src='https://via.placeholder.com/500x750?text=No+Image';}}/>
...
const ContentRow = React.memo(({title,fetchUrl,onMediaClick,apiKey,isLarge=false,customData=null,showRanking=false})=>{
  const [movies,setMovies]=useState([]), [fetched,setFetched]=useState(false), [intersecting,setIntersecting]=useState(false);
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        const [d1,d2]=await Promise.all([fetchWithCache(`${BASE_URL}${fetchUrl}${sep}api_key=${apiKey}&page=1`),fetchWithCache(`${BASE_URL}${fetchUrl}${sep}api_key=${apiKey}&page=2`)]);
        if(!ok)return;
  ...
  useEffect(()=>{
    if(fetched&&scrollRef.current&&loop){
      const t=setTimeout(()=>{if(scrollRef.current){scrollRef.current.style.scrollBehavior='auto';scrollRef.current.scrollLeft=getBlock();setTimeout(()=>{if(scrollRef.current)scrollRef.current.style.scrollBehavior='smooth';},50);}},50);
  ...
  return (
    <div ref={setRow} className="mb-10 md:mb-14 relative group/row animate-in fade-in slide-in-from-bottom-4 duration-700 optimize-row">
      <div className="flex items-center justify-between mb-4 md:mb-5 px-4 md:px-8 lg:px-12 xl:px-16 max-w-[1800px] mx-auto"><h2 className="text-xl md:text-2xl font-bold text-white/90 tracking-tight select-none">{title}</h2></div>
      <div className="relative group/nav max-w-[1800px] mx-auto">
        <button onClick={()=>scroll('left')} className="absolute left-0 top-0 bottom-0 w-16 md:w-24 bg-gradient-to-r from-black via-black/80 to-transparent z-30 hidden md:flex items-center justify-start pl-4 opacity-0 group-hover/nav:opacity-100 transition-all outline-none pointer-events-auto"><div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/10 border border-white/20 backdrop-blur-xl flex items-center justify-center hover:bg-[#19446C] hover:text-white text-white shadow-xl"><ChevronLeft className="w-5 h-5 md:w-8 md:h-8"/></div></button>
        <div ref={scrollRef} onScroll={onScroll} onMouseDown={mDown} onMouseLeave={mLeave} onMouseUp={mUp} onMouseMove={mMove} className={cn("flex gap-4 md:gap-6 overflow-x-auto pb-8 pt-4 px-4 md:px-8 lg:px-12 xl:px-16 cursor-grab active:cursor-grabbing hardware-accel snap-x snap-mandatory",showRanking&&"pl-8 md:pl-20")} style={{scrollBehavior:'smooth'}}>
          {display.map((m,i)=><MediaCard key={`${m.id}_${i}`} media={m} onClick={cardClick} size={isLarge?"large":"normal"} showRank={showRanking?i+1:null}/>)}
        </div>
        <button onClick={()=>scroll('right')} className="absolute right-0 top-0 bottom-0 w-16 md:w-24 bg-gradient-to-l from-black via-black/80 to-transparent z-30 hidden md:flex items-center justify-end pr-4 opacity-0 group-hover/nav:opacity-100 transition-all outline-none pointer-events-auto"><div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/10 border border-white/20 backdrop-blur-xl flex items-center justify-center hover:bg-[#19446C] hover:text-white text-white shadow-xl"><ChevronRight className="w-5 h-5 md:w-8 md:h-8"/></div></button>
      </div>
    </div>
  );
});
...
const Hero = React.memo(({onPlay,onMoreInfo,apiKey,type='all'})=>{
  ...
  const rand=useCallback(async()=>{
    ...
    try{
      ...
      const [d1,d2]=await Promise.all([fetchWithCache(`${BASE_URL}${ep}?api_key=${apiKey}&page=1`),fetchWithCache(`${BASE_URL}${ep}?api_key=${apiKey}&page=2`)]);
  ...
  return (
    <div className="relative w-full overflow-hidden group mb-12 flex flex-col justify-end min-h-[85vh] bg-black">
      <div className="absolute inset-0 bg-black"><img src={bg} alt="" className="w-full h-full object-cover object-top hardware-accel opacity-80 pointer-events-none" decoding="async" onError={()=>setImgFailed(true)} draggable="false"/></div>
      ...
      <div className="relative px-6 md:px-12 lg:px-16 pt-32 md:pt-52 pb-24 md:pb-40 flex flex-col justify-end z-20 w-full flex-1 pointer-events-none max-w-[1800px] mx-auto">
        <div className="max-w-4xl space-y-5 animate-in slide-in-from-bottom-8 fade-in duration-1000 delay-200 pointer-events-auto hardware-accel">
          <div className="flex items-center gap-3"><div className="bg-[#19446C] text-white border border-[#19446C]/50 px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-[0.2em]">{(movie.media_type==='tv'||type==='tv')?'Series':'Feature Film'}</div></div>
          ...
          <div className="flex items-center gap-4 text-xs md:text-sm font-medium text-white/70 select-none">
            <div className="flex items-center gap-1 font-semibold text-white bg-white/20 px-2.5 py-1 rounded-md backdrop-blur-md"><Star className="w-4 h-4 fill-yellow-500 text-yellow-500"/><span style={getRatingColor(movie.vote_average)}>{movie.vote_average?.toFixed(1)}</span></div>
...
const HomeView = React.memo(({apiKey,history,watchlist,algoPrefs,onPlay,onMoreInfo})=>{
  const [dynRows,setDynRows]=useState([]), [hasMore,setHasMore]=useState(true);
  const obs=useRef(null), used=useRef(new Set(['top_10','tmdb_top_movies','tmdb_top_tv','latest_action','latest_adventure','sci_fi','fantasy']));
  ...
  const genRow=useCallback(()=>{
    ...
    for(const s of src){const k=`rec_${s.id}`;if(!used.current.has(k)){used.current.add(k);const t=s.media_type||(s.name?'tv':'movie');return{id:k,title:`${history.some(h=>h.id===s.id)?'Because you watched':'Because you saved'} ${s.title||s.name}`,url:`/${t}/${s.id}/recommendations`};}}
  ...
  const lastRef=useCallback(node=>{
    ...
    obs.current=new IntersectionObserver(e=>{if(e[0].isIntersecting&&hasMore){setDynRows(prev=>{const nr=[];for(let i=0;i<3;i++){const r=genRow();if(r)nr.push(r);}if(!nr.length)setHasMore(false);return[...prev,...nr];});}},{rootMargin:'800px'});
...
const GenreGridView = React.memo(({apiKey,type,genreId,onMediaClick})=>{
  ...
  const lastRef=useCallback(node=>{
    if(loading)return; if(obs.current)obs.current.disconnect();
...
const MiniLiveStream = React.memo(({channel,isMain,isFocused,onSetFocus,onFullscreen,hasAudio,onRequestAudio,volume,onVolumeChange,showFullscreenButton,ccSettings})=>{
  ...
  useEffect(()=>{
    ...
    let inst;
    if(!window.Hls){const s=document.createElement('script');s.src="https://cdn.jsdelivr.net/npm/hls.js@latest";s.async=true;s.onload=()=>{inst=load();hlsRef.current=inst;};document.body.appendChild(s);}
    else{inst=load();hlsRef.current=inst;}
  ...
  useEffect(()=>{let t;if(inView&&!hasVideo&&!error&&channel.type==='m3u8'){t=setTimeout(()=>{if(hlsRef.current)hlsRef.current.startLoad();},15000);}return()=>clearTimeout(t);},[inView,hasVideo,error,channel.type]);
...
const LiveTvView = React.memo(({focusMode,setFocusMode,ccSettings})=>{
  ...
  const grid=()=>order.map((cid,i)=>{
    const ch=LIVE_CHANNELS.find(c=>c.id===cid);if(!ch||(viewMode==='sidebar'&&ch.id===focusedId))return null;
    return(
    ...
    <div className={cn("bg-black animate-in fade-in duration-700 flex flex-col transition-all ease-out px-4 md:px-8 lg:px-12 xl:px-16 max-w-[1800px] mx-auto",focusMode?"pt-4 md:pt-6":"pt-24 md:pt-32",(viewMode==='sidebar'||viewMode==='single')?"h-screen overflow-hidden pb-4":"min-h-screen pb-32")}>
      <div className={cn("flex items-center justify-end gap-3 md:gap-4 w-full shrink-0 py-1",focusMode?"mb-4":"mb-6 md:mb-8 mt-2")}>
        ...
        <div className="bg-white/10 backdrop-blur-2xl p-1.5 rounded-full flex gap-2 border border-white/10 shadow-lg">
          ...
          <button onClick={()=>setFocusMode(!focusMode)} className={cn("px-5 py-2.5 rounded-full text-xs md:text-sm font-semibold transition-all outline-none flex items-center gap-2 whitespace-nowrap",focusMode?"bg-[#19446C] text-white shadow-md":"text-white/70 hover:text-white hover:bg-white/20")}>
          ...
          <div key={i} className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 shrink-0 bg-white/10 rounded-full flex items-center justify-center p-1.5 border border-white/5">
                {t.badge?<img src={`https://streamed.pk/api/images/badge/${t.badge}.webp`} className="w-full h-full object-contain" alt={t.name} onError={e=>{e.target.style.display='none';e.target.nextSibling.style.display='block';}}/>:null}
...
// ─── PLAYER OVERLAY ───────────────────────────────────────────────────────
const PlayerOverlay=({media,config,onClose,sourceKey,apiKey})=>{
  const [loading,setLoading]=useState(true),[showCtrl,setShowCtrl]=useState(true),[resolvedId,setResolvedId]=useState(null),[src,setSrc]=useState('');
  const vRef=useRef(null),ctrlTimer=useRef(null),mounted=useRef(true);
  const isLiveMedia=media.isLive, isTv=!isLiveMedia&&(media.media_type==='tv'||media.name), cfg=config||{season:1,episode:1};
  useEffect(()=>{mounted.current=true;lockScroll();return()=>{mounted.current=false;unlockScroll();};},[]);
  useEffect(()=>{
    if(isLiveMedia){if(mounted.current)setResolvedId(media.url);return;}
    let ok=true;
    (async()=>{try{const t=media.media_type||(media.name?'tv':'movie');const d=await fetchWithCache(`${BASE_URL}/${t}/${media.id}/external_ids?api_key=${apiKey}`);if(ok&&mounted.current)setResolvedId(d.imdb_id||media.id);}catch{if(ok&&mounted.current)setResolvedId(media.id);}})();
    return()=>{ok=false;};
  },[media,isLiveMedia,apiKey]);
  const getSrc=useCallback((sk,id)=>{
    if(isLiveMedia)return media.url; if(!id)return'';
    const s=SOURCES[sk]||SOURCES.vidlink,u=s.url;
    if(sk==='vidlink'){const p=new URLSearchParams({primaryColor:'19446C',secondaryColor:'111111',iconColor:'ffffff',icons:'vid',title:'false',poster:'true',autoplay:'true',nextbutton:'true'}).toString();return isTv?`${u}/tv/${id}/${cfg.season}/${cfg.episode}?${p}`:`${u}/movie/${id}?${p}`;}
    if(sk==='vidking')return isTv?`${u}/tv/${id}/${cfg.season}/${cfg.episode}?color=19446C&autoPlay=true&nextEpisode=true&episodeSelector=true`:`${u}/movie/${id}?color=19446C&autoPlay=true`;
    if(sk==='embedsu')return isTv?`${u}/tv/${id}/${cfg.season}/${cfg.episode}`:`${u}/movie/${id}`;
    if(sk==='superembed')return isTv?`https://www.2embed.cc/embedtv/${media.id}&s=${cfg.season}&e=${cfg.episode}`:`https://www.2embed.cc/embed/${media.id}`;
    return isTv?`${u}/tv/${id}/${cfg.season}/${cfg.episode}`:`${u}/movie/${id}`;
  },[isTv,isLiveMedia,media,cfg.season,cfg.episode]);
  useEffect(()=>{if(isLiveMedia&&mounted.current)setSrc(media.url);else if(resolvedId&&mounted.current)setSrc(getSrc(sourceKey,resolvedId));if(mounted.current)setLoading(true);},[sourceKey,resolvedId,getSrc,isLiveMedia,media]);
  useEffect(()=>{
    if(!isLiveMedia||!src?.includes?.('.m3u8'))return;
    let hls;
    const load=()=>{if(mounted.current)setLoading(false);const v=vRef.current;if(!v)return;
      hls=initHls(v,src,true,{onParsed:()=>v.play().catch(()=>{}),onError:(e,d)=>{if(d.fatal){if(d.type===window.Hls?.ErrorTypes?.NETWORK_ERROR&&hls)hls.startLoad();else if(d.type===window.Hls?.ErrorTypes?.MEDIA_ERROR&&hls)hls.recoverMediaError();else if(hls)hls.destroy();}}});};
    if(!window.Hls){const s=document.createElement('script');s.src="https://cdn.jsdelivr.net/npm/hls.js@latest";s.async=true;s.onload=load;document.body.appendChild(s);}else load();
    return()=>{if(hls)hls.destroy();};
  },[isLiveMedia,src]);
  const mMove=useCallback(()=>{if(!showCtrl&&mounted.current)setShowCtrl(true);clearTimeout(ctrlTimer.current);ctrlTimer.current=setTimeout(()=>{if(mounted.current)setShowCtrl(false);},3000);},[showCtrl]);
  useEffect(()=>{window.addEventListener('mousemove',mMove);window.addEventListener('touchstart',mMove);ctrlTimer.current=setTimeout(()=>{if(mounted.current)setShowCtrl(false);},3000);return()=>{window.removeEventListener('mousemove',mMove);window.removeEventListener('touchstart',mMove);clearTimeout(ctrlTimer.current);};},[mMove]);
  return(
    <div className="fixed inset-0 z-[100] bg-black flex flex-col animate-in fade-in duration-500 font-sans hardware-accel">
      ...
      <div className={cn("absolute top-0 left-0 w-full p-4 md:p-8 flex items-start justify-between z-50 transition-opacity duration-300",showCtrl?"opacity-100":"opacity-0 pointer-events-none")}>
        <button onClick={onClose} className="flex items-center gap-2 text-white bg-white/10 hover:bg-[#19446C] px-6 py-3 rounded-full backdrop-blur-2xl transition-all border border-white/20 shadow-2xl group outline-none"><ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform"/><span className="text-sm font-semibold">Back</span></button>
        {isLiveMedia&&<div className="absolute left-1/2 -translate-x-1/2 top-6 flex items-center gap-2 bg-black/60 backdrop-blur-2xl border border-white/10 px-4 py-2 rounded-full"><div className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse"/><span className="text-xs font-semibold text-white uppercase tracking-widest">{String(media.name||'Live')}</span></div>}
      </div>
      <div className="flex-1 w-full h-full relative bg-black">
        {loading&&<div className="absolute inset-0 flex items-center justify-center bg-black z-0"><div className="w-12 h-12 border-4 border-[#19446C]/50 border-t-[#19446C] rounded-full animate-spin"/></div>}
        {isLiveMedia&&media.type==='iframe'&&<div className="w-full h-full relative bg-black flex flex-col"><iframe src={src} className="w-full h-full border-0 relative z-10" allowFullScreen allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture;fullscreen" onLoad={()=>{if(mounted.current)setLoading(false);}}/><div className="absolute bottom-6 right-6 z-50"><a href={src} target="_blank" rel="noopener noreferrer" className="bg-white/10 backdrop-blur-xl text-white hover:bg-[#19446C] px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 text-sm font-semibold border border-white/10"><ExternalLink className="w-4 h-4"/>Open in New Tab</a></div></div>}
        {isLiveMedia&&media.type==='livehdtv'&&<iframe src={src} className="w-full h-full border-0 relative z-10 bg-black" allowFullScreen allow="autoplay;encrypted-media;gyroscope;picture-in-picture;fullscreen" onLoad={()=>{if(mounted.current)setLoading(false);}}/>}
        {isLiveMedia&&src?.includes?.('.m3u8')&&<video ref={vRef} controls autoPlay playsInline className="w-full h-full border-0 relative z-10 bg-black outline-none"/>}
        {!isLiveMedia&&<iframe src={src} className="w-full h-full border-0 relative z-10 bg-black" allowFullScreen allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture;fullscreen" title="Video Player" onLoad={()=>{if(mounted.current)setLoading(false);}}/>}
      </div>
    </div>
  );
};
...
// ─── SEARCH VIEW ──────────────────────────────────────────────────────────
const SearchView=React.memo(({apiKey,history,onMediaClick,onPlaySport,onPlay})=>{
  const [mode,setMode]=useState('standard'),[q,setQ]=useState(''),dq=useDebounce(q,500);
  ...
  const lastRef=useCallback(node=>{if(isLoading||mode!=='standard')return;if(obs.current)obs.current.disconnect();obs.current=new IntersectionObserver(e=>{if(e[0].isIntersecting&&hasMore)setPage(p=>p+(dq?3:2));},{rootMargin:'800px'});if(node)obs.current.observe(node);},[isLoading,hasMore,dq,mode]);
      ...
      <div className="w-full relative z-30">
        ...
        {mode==='ai'&&(
          <div className="max-w-4xl mx-auto pb-32">
            <form onSubmit={async e=>{e.preventDefault();if(!aiP.trim()||aiLoad)return;setAiLoad(true);setAiErr('');setAiResp(null);try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:`${aiP}\nHistory: ${history.slice(0,15).map(h=>h.title||h.name).join(', ')}`}]}],systemInstruction:{parts:[{text:`You are a factual media assistant. Output JSON only. Today is ${new Date().toLocaleDateString()}.`}]},generationConfig:{responseMimeType:"application/json",responseSchema:{type:"OBJECT",properties:{text_response:{type:"STRING"},items:{type:"ARRAY",items:{type:"OBJECT",properties:{type:{type:"STRING"},search_query:{type:"STRING"},season_number:{type:"INTEGER"},episode_number:{type:"INTEGER"},literal_reason:{type:"STRING"}},required:["type","search_query","literal_reason"]}}},required:["text_response","items"]}}})});if(!r.ok)throw new Error();const txt=(await r.json()).candidates?.[0]?.content?.parts?.[0]?.text;if(txt){const j=JSON.parse(txt);const cards=[];for(const rec of(j.items||[])){try{if(rec.type==='movie'||rec.type==='tv'){const sr=await fetchWithCache(`${BASE_URL}/search/${rec.type==='tv'?'tv':'movie'}?api_key=${apiKey}&query=${encodeURIComponent(rec.search_query)}&include_adult=false`);const m=sr.results?.find(x=>x.poster_path);if(m)cards.push({ai_type:rec.type,media:m,reason:rec.literal_reason,id:`m-${m.id}`});}else if(rec.type==='live_tv'){const c=LIVE_CHANNELS.find(x=>x.name.toLowerCase().includes(rec.search_query.toLowerCase()));if(c)cards.push({ai_type:'live_tv',media:c,reason:rec.literal_reason,id:`l-${c.id}`});}}catch{}}setAiResp({text:(j.text_response||'').replace(/\*/g,''),cards});}}catch{setAiErr('Failed to generate a response. Please try again.');}setAiLoad(false);}} className="relative mb-12">
              <input type="text" placeholder="Ask about an episode, live channel, or get recommendations..." className="w-full bg-white/10 backdrop-blur-2xl border border-white/10 text-white text-lg py-5 pl-8 pr-16 rounded-3xl focus:bg-white/15 outline-none placeholder:text-white/40 focus:border-white/30 focus-visible:ring-4 focus-visible:ring-[#19446C] shadow-xl transition-all" value={aiP} onChange={e=>setAiP(e.target.value)} disabled={aiLoad} autoFocus/>
              <button type="submit" disabled={aiLoad||!aiP.trim()} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 text-white rounded-2xl bg-[#19446C] hover:bg-[#133250] transition-all disabled:opacity-50 outline-none">{aiLoad?<div className="w-5 h-5 border-4 border-white/30 border-t-white rounded-full animate-spin"/>:<Send className="w-5 h-5"/>}</button>
            </form>
                ...
                {aiResp.cards?.length>0&&(
                  <div className="grid grid-cols-1 gap-4 border-t border-white/10 pt-8">
                    {aiResp.cards.map(item=>(
                      <div key={item.id} className="flex flex-row gap-4 md:gap-6 p-4 md:p-6 bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 hover:border-white/30 transition-all group items-center shadow-lg cursor-pointer" onClick={()=>item.ai_type==='live_tv'?onPlay(item.media):onMediaClick(item.media)}>
                        {item.ai_type==='live_tv'?<div className="w-24 h-24 shrink-0 bg-white rounded-2xl flex items-center justify-center p-4"><img src={item.media.logo} className="w-full h-full object-contain" alt=""/></div>:<div className="w-24 md:w-32 shrink-0"><img src={`${IMAGE_BASE_URL}w200${item.media.poster_path}`} className="w-full rounded-2xl shadow-lg object-cover aspect-[2/3]" alt="" onError={e=>{e.target.src='https://via.placeholder.com/200x300?text=No+Image';}}/></div>}
                        <div className="flex-1"><div className="flex items-center gap-2 mb-2"><span className="bg-[#19446C] text-white px-2.5 py-1 rounded text-[10px] font-semibold uppercase tracking-widest">{item.ai_type==='live_tv'?'Live TV':item.ai_type==='tv'?'Series':'Movie'}</span></div><h4 className="text-xl font-bold text-white mb-2">{item.media.title||item.media.name}</h4><p className="text-white/60 text-sm line-clamp-2">{(item.reason||'').replace(/\*/g,'')}</p></div>
                      </div>
                    ))}
                  </div>
                )}
...
const PersonModal=({personId,apiKey,onClose,onMediaClick})=>{
  ...
  useEffect(()=>{if(!personId||!apiKey)return;let ok=true;(async()=>{try{const[d,c]=await Promise.all([fetchWithCache(`${BASE_URL}/person/${personId}?api_key=${apiKey}`),fetchWithCache(`${BASE_URL}/person/${personId}/combined_credits?api_key=${apiKey}`)]);if(!ok)return;setDet(d);setCred(Array.from(new Map((c.cast?.filter(x=>x.poster_path).sort((a,b)=>(b.popularity||0)-(a.popularity||0))||[]).map(i=>[i.id,i])).values()));}catch(e){console.error(e);}})();return()=>{ok=false;};},[personId,apiKey]);
  if(!det)return null;
  return(
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-0 md:p-12 animate-in fade-in duration-300 hardware-accel">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-3xl" onClick={onClose}/>
      <div className="relative bg-white/5 w-full h-full md:h-auto md:max-w-6xl md:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:max-h-[90vh] border border-white/10 animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 pt-24 md:p-16 flex flex-col md:flex-row gap-10 md:gap-16 overflow-y-auto">
          ...
          <div className="flex-1 space-y-10 md:space-y-14">
            ...
            <div><h3 className="text-xl font-bold text-white mb-6">Known For</h3><div className="grid grid-cols-3 sm:grid-cols-4 gap-4 sm:gap-6">{cred.slice(0,16).map(m=><MediaCard key={m.id} media={m} onClick={onMediaClick} size="grid"/>)}</div></div>
          </div>
        </div>
      </div>
    </div>
...
const Modal=({media,onClose,onPlay,isOpen,toggleWatchlist,isInWatchlist,apiKey,omdbApiKey,onCastClick,onMediaClick})=>{
  ...
  useEffect(()=>{
    ...
    setLoading(true);setDet(null);setOmdb(null);setTrailer(null);setEps([]);setSpoilers({});setShowTr(false);
    const saved=JSON.parse(localStorage.getItem('cloud_history')||'[]').find(i=>i.id===media.id);
    setSeason(saved?.config?.season||1);
    let ok=true;
    (async()=>{
      try{
        const t=media.media_type==='tv'||media.name?'tv':'movie';
  ...
  useEffect(()=>{
    if(!(media?.media_type==='tv'||media?.name)||media?.isLive||!apiKey||!season||!det||tab!=='episodes')return;
    let ok=true; setEpsLoad(true);
    (async()=>{try{const d=await fetchWithCache(`${BASE_URL}/tv/${media.id}/season/${season}?api_key=${apiKey}`);if(ok)setEps(d.episodes||[]);}catch{if(ok)setEps([]);}finally{if(ok)setEpsLoad(false);}})();
    return()=>{ok=false;};
  },[season,media,apiKey,det,tab]);
  if(!isOpen||!media||media.isLive)return null;
  const isTv=media.media_type==='tv'||media.name;
  ...
  return(
    <div className="fixed inset-0 z-[100] bg-black view-enter overflow-hidden font-sans modal-overlay-enter hardware-accel">
      {!showTr&&<button onClick={onClose} className="fixed top-6 right-6 md:top-10 md:right-10 p-4 bg-white/10 hover:bg-white hover:text-black backdrop-blur-2xl rounded-full text-white transition-all border border-white/10 z-[200] shadow-2xl cursor-pointer outline-none"><X className="w-6 h-6"/></button>}
      {showTr&&trailer&&(
      ...
      <div className="relative z-10 w-full h-full overflow-y-auto">
        <div className="min-h-screen flex flex-col justify-start pt-24 md:pt-40 px-6 md:px-12 lg:px-20 pb-32">
          <div className="flex flex-col md:flex-row gap-10 md:gap-20 max-w-[1800px] mx-auto w-full">
            <div className="w-48 md:w-[22rem] shrink-0 mx-auto md:mx-0"><img src={`${IMAGE_BASE_URL}${POSTER_SIZE}${media.poster_path}`} className="w-full rounded-2xl md:rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.8)] border border-white/10 object-cover aspect-[2/3]" alt="" decoding="async" onError={e=>{e.target.src='https://via.placeholder.com/500x750';}} /></div>
            <div className="flex-1 space-y-8 animate-in slide-in-from-right-10 fade-in duration-700 text-center md:text-left mt-4 md:mt-10">
              <div className="space-y-4">
                <div className="flex items-center justify-center md:justify-start gap-3">{isTv&&<span className="px-3 py-1 bg-[#19446C] rounded-md text-[10px] font-bold uppercase tracking-widest text-white">Series</span>}{det?.status&&<span className="text-[11px] font-semibold text-white/60 uppercase tracking-wider">{det.status}</span>}</div>
          ...
          <div className="max-w-[1800px] mx-auto w-full mt-20">
            <div className="flex items-center justify-center md:justify-start gap-10 border-b border-white/10 pb-4 mb-10 overflow-x-auto fade-edge-right">
              {['overview',isTv?'episodes':null,'similar'].filter(Boolean).map(t=>(
                <button key={t} onClick={()=>setTab(t)} className={cn("text-lg font-bold capitalize transition-all relative pb-2 shrink-0 outline-none",tab===t?"text-white":"text-white/50 hover:text-white/80")}>{t}{tab===t&&<div className="absolute -bottom-[3px] md:-bottom-[5px] left-0 w-full h-1 bg-white rounded-t-full"/>}</button>
              ))}
            </div>
                  ...
                  <div className="space-y-12">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-4">
                        {cast.map(p=>(
                          <button key={p.id} onClick={()=>onCastClick(p.id)} className="flex items-center gap-3 bg-white/5 hover:bg-white/10 backdrop-blur-md pr-6 rounded-full border border-white/5 transition-all group cursor-pointer outline-none hover:border-white/20 shadow-sm">
                            {p.profile_path?<img src={`${IMAGE_BASE_URL}w200${p.profile_path}`} className="w-14 h-14 rounded-full object-cover" alt="" decoding="async"/>:<div className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center"><User className="w-6 h-6 text-white/50"/></div>}
                            <div className="flex flex-col items-start py-2"><span className="text-sm font-semibold text-white/90 group-hover:text-white text-left">{p.name}</span><span className="text-xs text-white/50 text-left line-clamp-1">{p.character?.split('/')[0]}</span></div>
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              {tab==='episodes'&&isTv&&(
                <div className="animate-in fade-in slide-in-from-bottom-4 space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {epsLoad?<div className="col-span-full py-16 text-center text-white/50">Loading episodes...</div>:eps.length>0?eps.map(ep=>{
                      ...
                      return(
                        <div key={ep.id} className="group flex flex-col gap-4 p-4 bg-white/5 rounded-3xl hover:bg-white/10 backdrop-blur-lg transition-all border border-white/10 shadow-sm">
                          <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black/50">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur-[2px]"><button onClick={()=>onPlay(media,{season,episode:ep.episode_number})} className="bg-white text-black p-4 rounded-full hover:scale-110 transition-transform shadow-2xl outline-none"><Play className="w-6 h-6 fill-current"/></button></div>
...
export default function App() {
  ...
  // Destructure settings for ergonomics
  ...
  useEffect(()=>{if(!requireProfile)setProfileSelected(true);},[requireProfile]);
  useEffect(()=>{ setSelGenre('All'); if(activeTab!=='live')setFocusMode(false); },[activeTab]);
  ...
  const effAlgo=useMemo(()=>({boosted:[...new Set([...algoPrefs.boosted,...autoBoosted])].filter(id=>!algoPrefs.excluded.includes(id)),excluded:algoPrefs.excluded}),[algoPrefs,autoBoosted]);

  const toggleAlgo=useCallback(id=>setAlgoPrefs(p=>{ if(p.boosted.includes(id))return{boosted:p.boosted.filter(x=>x!==id),excluded:[...new Set([...p.excluded,id])]}; if(p.excluded.includes(id))return{boosted:p.boosted,excluded:p.excluded.filter(x=>x!==id)}; return{boosted:[...new Set([...p.boosted,id])],excluded:p.excluded}; }),[setAlgoPrefs]);
  ...
  const handlePlay=useCallback((media,config=null)=>{
    ...
    if((media.media_type==='tv'||media.name)){
  ...
  const handlePlaySport=useCallback(match=>new Promise(async resolve=>{
    ...
    if(!url){for(const src of match.sources){if(src.source==='daddyhd'||src.source==='daddy')continue;try{const r=await fetch(`https://streamed.pk/api/stream/${src.source}/${src.id}`);if(!r.ok)continue;const ss=await r.json();const v=Array.isArray(ss)?ss:(typeof ss==='object'&&ss?Object.values(ss):[]);if(v.length){const b=v.find(s=>s.language==='English'&&s.hd)||v[0];if(b.embedUrl){url=b.embedUrl;isIfr=true;break;}else if(b.streamUrl||b.url){url=b.streamUrl||b.url;isIfr=!url.includes('.m3u8');break;}}}catch{}}}
  ...
  if(!profileSelected&&requireProfile) return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center font-sans px-6">
      <h1 className="text-4xl md:text-5xl font-bold text-white mb-16 tracking-tight">Who's watching?</h1>
      <div className="flex flex-wrap items-center justify-center gap-10 md:gap-16">
        {['Main','Guest'].map((name,i)=>(
          <button key={name} onClick={()=>setProfileSelected(true)} className="flex flex-col items-center gap-6 cursor-pointer group outline-none">
            <div className={cn("w-32 h-32 md:w-40 md:h-40 rounded-[2.5rem] transition-all duration-300 border-4 flex items-center justify-center",i===0?"bg-gradient-to-br from-zinc-700 to-zinc-900 border-transparent group-hover:border-[#19446C] shadow-2xl":"bg-white/10 backdrop-blur-xl border-transparent group-hover:border-[#19446C] shadow-xl")}><User className="w-14 h-14 md:w-16 md:h-16 text-white/50"/></div>
            <span className="text-white/50 font-semibold text-lg md:text-xl group-hover:text-white transition-colors">{name}</span>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-[#19446C] selection:text-white overflow-x-hidden">
      <GlobalStyles/>
      <TopNavigation activeTab={activeTab} setActiveTab={setActiveTab} selectedGenre={selGenre} setSelectedGenre={setSelGenre} focusMode={focusMode} onSignOut={()=>firebaseSignOut(auth)}/>
      <main className="transition-all duration-700 ease-out">
        ...
        {activeTab==='movies'&&(
          <div className="pb-32 bg-black min-h-screen">
            {selGenre==='All'?(<><Hero onPlay={handlePlay} onMoreInfo={setSelMedia} apiKey={apiKey} type="movie"/>
              <div className="relative z-20 -mt-24 md:-mt-32">
        ...
        {activeTab==='tv'&&(
          <div className="pb-32 bg-black min-h-screen">
            {selGenre==='All'?(<><Hero onPlay={handlePlay} onMoreInfo={setSelMedia} apiKey={apiKey} type="tv"/>
              <div className="relative z-20 -mt-24 md:-mt-32">
        ...
        {activeTab==='search'&&<SearchView apiKey={apiKey} history={history} onMediaClick={setSelMedia} onPlay={handlePlay} onPlaySport={handlePlaySport}/>}
        {activeTab==='watchlist'&&(
          <div className="px-4 md:px-8 lg:px-12 xl:px-16 min-h-screen bg-black max-w-[1800px] mx-auto animate-in fade-in duration-500 pt-24 md:pt-40">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4 md:gap-6 lg:gap-8">{watchlist.map(i=><MediaCard key={i.id} media={i} onClick={setSelMedia} size="grid"/>)}</div>
                  ...
                  <div key={item.id} className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 hover:border-[#19446C]/50 transition-all group shadow-lg overflow-hidden flex flex-col hardware-accel">
                    <div className="flex gap-4 md:gap-8 p-4 md:p-6 items-start">
                      ...
                      <div className="flex items-center gap-2 md:gap-4 px-2 shrink-0 h-full pt-2">
                        <button onClick={()=>handlePlay(item,item.config)} className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-white text-black flex items-center justify-center opacity-100 md:opacity-0 group-hover:opacity-100 transition-all hover:scale-105 shadow-xl outline-none"><Play className="w-5 h-5 fill-current ml-1"/></button>
                    ...
                    {hasEps&&(
                      <div className="border-t border-white/10 bg-black/20">
                        <button onClick={()=>setExpHistory(p=>({...p,[item.id]:!p[item.id]}))} className="w-full py-3 text-xs font-semibold text-white/50 hover:text-white flex items-center justify-center gap-2 outline-none"><ChevronDown className={cn("w-4 h-4 transition-transform",isExp&&"rotate-180")}/>{isExp?'Hide Episodes':`View Episode History (${item.watchedEpisodes.length})`}</button>
                        {isExp&&<div className="flex flex-col pb-4 px-6">{item.watchedEpisodes.map(ep=>(
                          <div key={`${ep.season}-${ep.episode}`} className="flex items-center justify-between py-3 border-t border-white/5">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4"><span className="text-sm font-semibold text-white/80">Season {ep.season} Episode {ep.episode}</span><span className="text-xs text-white/40">{formatHistoryDate(ep.watchedAt)}</span></div>
          ...
          <div className="px-4 md:px-8 lg:px-12 xl:px-16 min-h-screen max-w-5xl mx-auto animate-in fade-in duration-500 pt-24 md:pt-32">
            ...
            <div className="grid gap-6 md:gap-8 pb-32">
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:border-[#19446C]/50 shadow-lg">
                ...
                <button onClick={()=>setRequireProfile(!requireProfile)} className={cn("w-14 h-8 rounded-full transition-colors relative shrink-0 outline-none",requireProfile?"bg-[#19446C]":"bg-white/20")}><div className={cn("w-6 h-6 bg-white rounded-full absolute top-1 transition-transform shadow-sm",requireProfile?"translate-x-7":"translate-x-1")}/></button>
              </div>
              {/* CC Settings */}
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl hover:border-[#19446C]/50 shadow-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {[
                    ...
                    <select key={i} value={val} onChange={e=>setter(e.target.value)} className="custom-select bg-black/40 text-white border border-white/10 rounded-2xl px-5 py-4 outline-none focus-visible:border-[#19446C] cursor-pointer text-sm font-semibold">{opts.map(([v,l])=><option key={v} value={v} className="bg-black text-white">{l}</option>)}</select>
                  ))}
                </div>
              </div>
              {/* Algorithm Prefs */}
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl hover:border-[#19446C]/50 shadow-lg">
                ...
                <p className="text-sm text-white/60 mb-6">Primary server for video playback.</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {Object.entries(SOURCES).map(([k,v])=>(
                    <button key={k} onClick={()=>setSourceKey(k)} className={cn("px-5 py-4 rounded-2xl text-left border transition-all flex items-center justify-between group outline-none",sourceKey===k?"bg-white text-black border-white shadow-lg":"bg-black/40 border-white/10 text-white/60 hover:border-[#19446C]/50 hover:text-white")}>
                      <span className="font-bold text-base">{v.name}</span><span className={cn("text-[10px] uppercase font-bold tracking-widest px-2 py-1 rounded-md",sourceKey===k?"bg-black/10 text-black":"bg-white/10 text-white/50")}>{v.type}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl hover:border-[#19446C]/50 shadow-lg">
                ...
                <p className="text-xs text-white/50 mt-3 flex items-center gap-2"><Monitor className="w-3.5 h-3.5"/>Synced securely to your account.</p>
              </div>
              {/* OMDb key */}
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl hover:border-[#19446C]/50 shadow-lg">
                ...
                <p className="text-sm text-white/60 mb-6">Free key from <a href="https://www.omdbapi.com" target="_blank" className="text-white underline hover:text-white/80">omdbapi.com</a> for IMDb, RT, and Metacritic ratings.</p>
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-3xl flex items-center justify-between hover:border-red-500/30 shadow-lg">
                <div><h3 className="text-lg md:text-xl font-bold text-white flex items-center gap-3 mb-1"><LogOut className="w-5 h-5 text-red-400"/>Sign Out</h3><p className="text-sm text-white/60">Signed in as {user.email}</p></div>
      ...
      <Modal isOpen={!!selMedia} media={selMedia} onClose={()=>setSelMedia(null)} onPlay={handlePlay} toggleWatchlist={toggleWatchlist} isInWatchlist={selMedia?wlRef.current.some(m=>m.id===selMedia.id):false} apiKey={apiKey} omdbApiKey={omdbApiKey} onCastClick={setSelPerson} onMediaClick={setSelMedia}/>
      ...
      {playMedia&&<PlayerOverlay media={playMedia} config={playCfg} onClose={()=>setPlayMedia(null)} sourceKey={sourceKey} apiKey={apiKey}/>}
    </div>
  );
}
